...
if __name__ == "__main__":
    ...
    # Upload annotations. Here, we first need to check whether the project has
    # multiple tasks
    task_parameter_list = project_manager.get_label_names_per_task(project)

    # Upload annotations for all tasks
    for task_index, task_parameters in enumerate(task_parameter_list):
        task_type = list(task_parameters.keys())[0]
        annotation_reader = SCAnnotationReader(
            base_data_folder=os.path.join(TARGET_FOLDER, "annotations"),
            task_type=task_type,
            task_type_to_label_names_mapping=task_parameters
        )

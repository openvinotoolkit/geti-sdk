...
def _image_to_np(image: Union[np.ndarray, Image],
                 ) -> np.ndarray:
    """
...
class TransformImages:
    ...
    def __init__(self, corruption_type: str = 'motion_blur',
                 ):
        """
        ...
        self._SEED = 117
        self._SUPPORTED_CORRUPTIONS = ['motion_blur', 'gaussian_blur', 'cut_out', 'fake_snow', 'poisson_noise']
        ...
        if self.corruption_type in ['cut_out', 'fake_snow']:
            ...
            self._corruption_strength_range = (1, 110)  # Increase this if the desired accuracy is not reached
        elif self.corruption_type == 'gaussian_blur':
            self._corruption_strength_range = (1, 8)    # Increase this if the desired accuracy is not reached
        elif self.corruption_type == 'motion_blur':
            self._corruption_strength_range = (1, 110)  # Increase this if the desired accuracy is not reached

    def _compose_imgaug_corruption(self,
                                   corruption_strength: Union[float, int]
                                   ):
        transform = None
        if self.corruption_type == 'motion_blur':
            transform = iaa.MotionBlur(k=int(round(corruption_strength)),
                                       angle=45,
                                       direction=0.5,
                                       seed=self._SEED)

        elif self.corruption_type == 'gaussian_blur':
            transform = iaa.GaussianBlur(sigma=corruption_strength,
                                         seed=self._SEED)

        elif self.corruption_type == 'cut_out':
            transform = iaa.Cutout(nb_iterations=2,  # No. of boxes per image
                                   size=corruption_strength,
                                   squared=False,
                                   seed=self._SEED,
                                   position='uniform',  # "uniform" - random, "normal" - center of the image
                                   fill_mode="constant",
                                   cval=0)  # fill with black boxes

        elif self.corruption_type == 'fake_snow':
            # SnowFlakes from ImgAug can give error depending on the numpy version you have.
            transform = iaa.Snowflakes(density=0.075,
            ...
            transform = iaa.AdditivePoissonNoise(lam=corruption_strength,
                                                 per_channel=True,
                                                 seed=self._SEED)

        return transform

    def _apply_corruption_on_image(self,
        ...
        transformation = self._compose_imgaug_corruption(corruption_strength=corruption_strength)
        transformed_image = transformation(image=input_image)
        return transformed_image

    def apply_corruption_on_folder(self,
                                   source_path: str,
                                   dest_path: str,
                                   corruption_strength: Union[float, int],
        ...
        for folder_name in tqdm(os.listdir(source_path), disable=not show_progress, desc=f"Applying Corruption"):
            ...
            for image_name in os.listdir(class_folder_path):
                ...
                transformed_image = self._apply_corruption_on_image(image, corruption_strength=corruption_strength)
                # Save the transformed image to the destination folder
                cv2.imwrite(os.path.join(dest_folder, image_name), transformed_image)
        return dest_path

    def generate_ood_dataset_by_corruption(self,
        ...
        """
        Generates a dataset of corrupted images from a source dataset of clean images.
        ...
        """
        print(f"Generating OOD dataset by applying {self.corruption_type} corruption on {source_path}")
        ...
        accuracy = calc_classification_accuracy(dataset_path=source_path,
        ...
        while abs(accuracy - desired_accuracy) > desired_accuracy_tol:
            corruption_strength = self._update_corruption_strength(desired_accuracy=desired_accuracy,
                                                                   current_accuracy=accuracy,
                                                                   current_strength=corruption_strength)
            self.apply_corruption_on_folder(source_path=source_path,
                                            dest_path=dest_path,
                                            corruption_strength=corruption_strength,
                                            show_progress=show_progress)
            accuracy = calc_classification_accuracy(dataset_path=dest_path,
                                                    deployment=geti_deployment,
                                                    show_progress=show_progress)
            if show_progress:
    ...
    def _update_corruption_strength(
        ...
        """
        Updates the corruption strength based on the current accuracy and the desired accuracy.
        """
        ...
        accuracy_diff = ((current_accuracy - desired_accuracy) / 100) * (limit[1] - limit[0])
...
def calc_classification_accuracy(
    ...
    """
    Calculates the classification accuracy of a dataset using a trained Geti deployment.
    ...
    for image_path, label in tqdm(id_images_dict.items(),
...
def extract_features_from_imageclient(
    ...
    """
    Extracts feature embeddings from a Geti deployment model for a given number of images in a geti image_client
    ...
    for i, k in tqdm(enumerate(random_indices), total=n_images, desc="Extracting features"):
...
def extract_features_from_img_folder(
    ...
    """
    Extracts feature embeddings from a Geti deployment model for images in a folder
    :param deployment: The trained Geti deployment (model) to use for feature extraction.
    ...
    for k, id_image_path in tqdm(enumerate(images_in_folder.keys()), total=len(images_in_folder.keys()),
...
def get_grid_arrangement(n: int):
    """
    Returns the number of rows and columns for a grid arrangement of n items in a grid.
    ...
    index_closest_to_sqrt = min(range(len(factors)), key=lambda i: abs(factors[i] - sqrt(n)))
...
def display_sample_images_in_folder(images_path: str,
                                    title: str = None,
                                    n_images: int = 9,
                                    show_labels: bool = True):
    """
    Displays a random sample of images from a dataset folder
    ...
    random_indices = np.random.choice(len(images_in_folder.keys()), n_images, replace=False)
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        ax.imshow(image)
        if show_labels:
            ax.set_title(list(images_in_folder.values())[random_indices[i]],
                         color='#0068b5',

...
class AnomalyClassification(SegmentationModel):
        ...
        parameters.update(
            {
                "image_threshold": NumericalValue(
                    description="Threshold value to locate anomaly"
                ),
                "min": NumericalValue(
                    description="Threshold value to locate anomaly"
                ),
                ...
                "threshold": NumericalValue(
                    description="Threshold used to classify anomaly"
                ),
            }
        )
    ...
    def _normalize(
        ...
        """
        Apply min-max normalization and shift the values such that the threshold
        value is centered at 0.5.
        """
        ...
        if isinstance(targets, (np.ndarray, np.float32)):
            ...
            raise ValueError(
                f"Targets must be either Tensor or Numpy array. Received "
                f"{type(targets)}"
            )
        return normalized
    ...
    def postprocess(
        ...
        Args:
            outputs (Dict[str, np.ndarray]): Raw outputs of the model
                after ``infer_sync`` is called.
            meta (Dict[str, Any]): Metadata which contains values such as threshold,
                original image size.
        ...
        anomaly_map = self._normalize(
            anomaly_map, meta["image_threshold"], meta["min"], meta["max"]
        )
        pred_score = self._normalize(
            pred_score, meta["image_threshold"], meta["min"], meta["max"]
        )
